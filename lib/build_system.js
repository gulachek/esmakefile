...
class BuildSystem {
	...
	ext(absPath) {
		return this.path(this.#fs.ext(absPath));
	}

	install(path) {
		if (path instanceof Target) { return path; }
		return this.path(this.#fs.install(path));
	}

	src(path) {
		if (path instanceof Target) { return path; }
		return this.path(this.#fs.src(path));
	}

	dest(path) {
		if (path instanceof Target) { return path; }
		return this.path(this.#fs.dest(path));
	}

	cache(path, args) {
		return this.path(this.#fs.cache(path, args));
	}
	...
	path(p) {
		...
		if (p instanceof Target) {
			if (!(p instanceof PathTarget)) {
				throw new Error(`${p} is not a PathTarget`);
			}
