...
class Path {
	...
	dir() {
		const components = this.#components.slice(0, this.#components.length-1);
	...
	abs(p) {
		...
		switch (p.base) {
			...
			case 'sys':
				...
				break;
			case 'install':
				base = process.env.GULPACHEK_INSTALL_ROOT;
				if (!base) {
					throw new Error(`GULPACHEK_INSTALL_ROOT is not defined`);
				}
				break;
			default:
		...
		return this.#path.resolve(base, ...p.components);
	}
}
